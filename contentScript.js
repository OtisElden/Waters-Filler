...
// This function loads the Tesseract script if it's not already loaded
...
async function initializeTesseract() {
    await loadTesseractScript();  // Ensure the Tesseract script is loaded

    tessWorker = Tesseract.createWorker({
        workerPath: chrome.runtime.getURL("scripts/tesseract.worker.min.js"),
        corePath: chrome.runtime.getURL("scripts/tesseract-core.wasm.js"),
...
async function handleFile(file) {
    ...
    try {
        const { data } = await tessWorker.recognize(file);
        console.log(data.text); // You can process the OCR result here
    } catch (error) {
        console.error(error);
    }
}

// Watch for a specific button and trigger OCR when it's found
function patientWatcher() {
    const intervalID = setInterval(() => {
        const buttons = document.querySelectorAll(".button-control");
        const foundButton = Array.from(buttons).find(button => button.textContent.trim() === "Find a Repeat Patient");

        if (foundButton) {
            console.log("Found button, starting OCR");
            handleFile("testocr.png"); // Replace 'testocr.png' with the actual file you want to process
            clearInterval(intervalID);
        }
    }, 1000);
}

// Start the watcher and initialize Tesseract
patientWatcher();
initializeTesseract();
